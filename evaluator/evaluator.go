...
func evalHashLiteral(node *ast.HashLiteral, env *object.Environment) object.Object {
	pairsObjects := make(map[object.HashKey]object.HashPair)
	for key, val := range node.Pairs {
		keyEvaluated := Eval(key, env)
		if isError(keyEvaluated) {
			return keyEvaluated
		}
		hashable, ok := keyEvaluated.(object.Hashable)
		if !ok {
			return newError("unusable as hash key: %s", keyEvaluated.Type())
		}
		hash := hashable.HashKey()
		valEvaluated := Eval(val, env)
